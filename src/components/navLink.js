...
const NavLink = ({ to, onDark, children }) => {
  ...
  const activeClasses = `underline ${onDark ? "text-gray-min-lvl" : "text-gray-max-lvl"}`;

  return (
    <Link to={to} className={navLinkClasses} activeClassName={activeClasses} aria-label={children}>
      {children}
    </Link>
  );
};
