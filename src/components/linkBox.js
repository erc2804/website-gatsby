...
const LinkBox = ({ label, url, descText, type, categoryDesc, children }) => {
  ...
  if(React.isValidElement(firstChild)) {
    if(firstChild.type === GatsbyImage) {
      boxVisualClasses += " [&>div[data-gatsby-image-wrapper]]:size-full pt-6 lg:group-hover:-translate-y-3"
    } else {
      boxVisualClasses += " flex items-center justify-center h-[calc(100%-98px)] [&>svg]:size-3/5 lg:group-hover:-translate-y-3 " + ColorClassesPerCategory[type].icon
    }
  }
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 flex flex-col gap-2 px-4 pt-4 pb-5 w-full bg-gray-min-lvl">
        <div className="flex flex-row gap-x-4 justify-between items-center">
          ...
          {categoryDesc &&
            <div className={`flex-none px-1.5 py-0.5 rounded-full ec-font-caption font-bold ${ColorClassesPerCategory[type].bgAlt} ${ColorClassesPerCategory[type].text}`}>{categoryDesc}</div>
          }
        </div>
