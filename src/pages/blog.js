        ...
        <div className="flex flex-col gap-10">
          {blogPosts.map((blogPost, idx) => (
            <article key={idx} className="w-full">
              <button className="w-full h-52 rounded-md overflow-hidden relative group" onClick={() => toggleBlogEntry(blogPost.id)}>
                <div
                ...
                ></div>
                <div className={`absolute inset-0 duration-500 z-10 ${blogPost.isOpen ? 'bg-transparency-low-lvl' : 'bg-transparency-medium-lvl xl:group-hover:bg-transparency-low-lvl'}`} />
                <div className="p-2 flex flex-row gap-6 items-end absolute inset-0 z-20">
                  <div className={`flex-1 flex flex-col gap-3 ${blogPost.isOpen ? 'translate-y-full' : ''} transition-transform`}>
                  ...
                  <ChevronDownIcon iconClasses={`flex-none size-6 fill-brand-sand ${blogPost.isOpen ? 'rotate-180 xl:group-hover:rotate-[168deg]' : 'xl:group-hover:rotate-12'} transition-transform `} />
                </div>
              </button>
              <div className={`flex flex-col ec-layout-text-content ${blogPost.isOpen ? '' : 'hidden'}`}>
                <h2 className="flex flex-row gap-3 justify-between mt-7 text-left">
                  <span className="flex-1 ec-font-heading-1 first:pr-4">{blogPost.title}</span>
